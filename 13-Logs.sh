...
if [ $? -ne 0 ]
then
  echo -e "$Y Mysql is not installed, Installation will Proceed $N" | tee -a $LOG_FILE
  dnf install mysql -y
  #This is install status after it is being installed
  VALIDATE $? "MYSQL" #here $1 = $? (1st argument) $2 = MYSQL (2nd Argument) 
else
   echo -e "$N MYSQL is already installed , Nothing to do! $N" | tee -a $LOG_FILE
fi
...
if [ $? -ne 0 ]
then
  echo -e "$Y python3 is not installed, Installation will Proceed $N" | tee -a $LOG_FILE
  ...
  VALIDATE $? "Python3" #here $1 = $? (1st argument) $2 = Python3 (2nd Argument)
   ...
   echo -e "$N Python3 is already installed , Nothing to do! $N" | tee -a $LOG_FILE
fi
...
if [ $? -ne 0 ]
then
  echo -e "$Y Nginx is not installed, Installation will Proceed $N" | tee -a $LOG_FILE
  #If  not installed proceed with installation
  dnf install nginx -y
   #This is install status after it is being installed
  VALIDATE $? "Nginx" #here $1 = $? (1st argument) $2 = Nginx (2nd Argument)
   ...
   echo -e "$N Nginx is already installed , Nothing to do! $N" | tee -a $LOG_FILE
fi